...
| Per-target cooldown | **Simplified** | Enforced, and enforced across payers so paying repeatedly is not unlimited flooding — but held in memory, so a server restart clears it. Persistence is a Day 9 concern |
